    ...
    >
      ...
      <section className="content-grid split-65">
        ...
        <SectionCard title="Organization setup & branding" eyebrow="Company profile and document controls">
          ...
          <div className="action-form-stack"><div className="company-branding-preview">{logoUrl ? <Image src={logoUrl} alt={`${activeCompany?.name ?? "Company"} logo`} className="company-logo-preview" width={64} height={64} unoptimized /> : <div className="company-logo-placeholder">No logo</div>}<div><strong>{activeCompany?.name ?? "No active company selected"}</strong><p className="cell-subcopy">Update core identity, numbering, and statutory defaults here.</p></div></div><div className="form-grid two-up"><label className="field"><span>Registered name</span><input value={state.companyProfile.registeredName} onChange={(event) => setState((current) => ({ ...current, companyProfile: { ...current.companyProfile, registeredName: event.target.value } }))} /></label><label className="field"><span>Tax ID / TIN</span><input value={state.companyProfile.taxId} onChange={(event) => setState((current) => ({ ...current, companyProfile: { ...current.companyProfile, taxId: event.target.value } }))} /></label></div><div className="form-grid two-up"><label className="field"><span>Timezone</span><input value={state.companyProfile.timezone} onChange={(event) => setState((current) => ({ ...current, companyProfile: { ...current.companyProfile, timezone: event.target.value } }))} /></label><label className="field"><span>Base currency</span><input value={state.companyProfile.baseCurrency} onChange={(event) => setState((current) => ({ ...current, companyProfile: { ...current.companyProfile, baseCurrency: event.target.value } }))} /></label></div><label className="field"><span>Logo URL</span><input value={logoUrl} onChange={(event) => setLogoUrl(event.target.value)} /></label><label className="field"><span>Upload logo</span><input type="file" accept="image/*" onChange={handleLogoUpload} /></label><div className="form-grid two-up"><label className="field"><span>Invoice prefix</span><input value={state.numbering.invoice} onChange={(event) => setState((current) => ({ ...current, numbering: { ...current.numbering, invoice: event.target.value } }))} /></label><label className="field"><span>Voucher prefix</span><input value={state.numbering.voucher} onChange={(event) => setState((current) => ({ ...current, numbering: { ...current.numbering, voucher: event.target.value } }))} /></label></div><div className="inline-actions compact-end"><button className="primary-button" type="button" onClick={saveCompanyProfile}>Save company profile</button><button className="ghost-button" type="button" onClick={() => persist(state, "Browser-only admin preferences updated. They have not been written to the ERP database yet.")}>Save controls (browser only)</button></div></div>
        </SectionCard>
      </section>
      ...
      <section className="content-grid split-65">
        ...
        <SectionCard title="Security, notifications, integrations, and audit" eyebrow="Policy visibility">
          ...
          <div className="mfa-setup-wrapper" style={{ marginBottom: "1.5rem" }}>
            {session?.token && (
              <MfaSetup token={session.token} apiBaseUrl={process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:3000/api/v1"} />
            )}
          </div>
